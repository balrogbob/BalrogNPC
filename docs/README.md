# BalrogNPC Documentation

**Welcome to the complete documentation for BalrogNPC!**

---

## ?? Start Here

### New to BalrogNPC?
?? **[Getting Started Guide](GETTING_STARTED.md)** - Quick installation and first steps

### Looking for something specific?
?? **[Documentation Index](INDEX.md)** - Complete navigation and search

### Want to learn rAthena scripting?
?? **[rAthena Script Guide](RATHENA_SCRIPT_GUIDE.md)** - 9-chapter comprehensive guide

---

## ?? Documentation Files

### Getting Started
- **[Getting Started](GETTING_STARTED.md)** - Installation, first launch, your first NPC (30 min)
- **[User Guide](USER_GUIDE.md)** - Complete feature walkthrough (2 hours)
- **[Keyboard Shortcuts](KEYBOARD_SHORTCUTS.md)** - All keyboard shortcuts reference

### rAthena Scripting
- **[rAthena Script Guide](RATHENA_SCRIPT_GUIDE.md)** - Complete 9-chapter guide (6-8 hours)
- **[Quick Reference](QUICK_REFERENCE.md)** - One-page command reference
- **[Script Examples](SCRIPT_EXAMPLES.md)** - 10 working code examples

### Support
- **[Troubleshooting](TROUBLESHOOTING.md)** - Common issues and solutions
- **[FAQ](FAQ.md)** - Frequently asked questions

### Navigation
- **[Index](INDEX.md)** - Complete documentation index with learning paths
- **[Documentation Summary](DOCUMENTATION_SUMMARY.md)** - Overview of all documentation

---

## ?? Quick Links by Task

| I want to... | Go to... |
|-------------|----------|
| **Install BalrogNPC** | [Getting Started](GETTING_STARTED.md#installation) |
| **Create my first NPC** | [Getting Started](GETTING_STARTED.md#your-first-npc) |
| **Learn all features** | [User Guide](USER_GUIDE.md) |
| **Find a command** | [Quick Reference](QUICK_REFERENCE.md) |
| **See code examples** | [Script Examples](SCRIPT_EXAMPLES.md) |
| **Learn rAthena scripting** | [rAthena Script Guide](RATHENA_SCRIPT_GUIDE.md) |
| **Find keyboard shortcuts** | [Keyboard Shortcuts](KEYBOARD_SHORTCUTS.md) |
| **Solve a problem** | [Troubleshooting](TROUBLESHOOTING.md) |
| **Get a quick answer** | [FAQ](FAQ.md) |

---

## ?? Documentation Stats

- **Total Files:** 10
- **Total Size:** ~92 KB
- **Estimated Pages:** ~210
- **Total Read Time:** ~15 hours
- **Code Examples:** 100+
- **Working NPC Examples:** 10

---

## ??? Learning Paths

### Beginner (2-3 hours)
1. [Getting Started](GETTING_STARTED.md)
2. [Keyboard Shortcuts](KEYBOARD_SHORTCUTS.md)
3. [Quick Reference](QUICK_REFERENCE.md)
4. [Script Examples](SCRIPT_EXAMPLES.md)

### Intermediate (6-8 hours)
1. [User Guide](USER_GUIDE.md)
2. [rAthena Script Guide](RATHENA_SCRIPT_GUIDE.md) - Chapters 1-3
3. [Script Examples](SCRIPT_EXAMPLES.md)
4. Practice creating NPCs

### Advanced (12-15 hours)
1. Complete [rAthena Script Guide](RATHENA_SCRIPT_GUIDE.md)
2. All [Script Examples](SCRIPT_EXAMPLES.md)
3. Master all BalrogNPC tools
4. Create complex custom NPCs

---

## ?? Search Tips

1. **Use the Index** - [INDEX.md](INDEX.md) has comprehensive navigation
2. **Use browser search** - Ctrl+F in GitHub viewer
3. **Use grep** - Search all files: `grep -r "search term" docs/`
4. **Check FAQ** - [FAQ.md](FAQ.md) for common questions

---

## ?? Documentation Format

All documentation is in **Markdown (.md)** format and can be:
- ? Viewed in any text editor
- ? Rendered in GitHub/GitLab
- ? Opened in VS Code with preview
- ? Converted to PDF or HTML
- ? Searched with text tools

---

## ?? Need Help?

1. **Check the Index** - [INDEX.md](INDEX.md)
2. **Check the FAQ** - [FAQ.md](FAQ.md)
3. **Check Troubleshooting** - [TROUBLESHOOTING.md](TROUBLESHOOTING.md)
4. **Search documentation** - Use Ctrl+F or grep
5. **Ask on GitHub** - [Create an issue](https://github.com/balrogbob/BalrogNPC/issues)

---

## ?? File List

```
docs/
??? README.md                    ? You are here
??? INDEX.md                     ? Complete navigation
??? GETTING_STARTED.md           ? Installation & first steps
??? USER_GUIDE.md                ? Complete feature guide
??? KEYBOARD_SHORTCUTS.md        ? Shortcuts reference
??? QUICK_REFERENCE.md           ? Command reference
??? RATHENA_SCRIPT_GUIDE.md      ? 9-chapter scripting guide
??? SCRIPT_EXAMPLES.md           ? Working code examples
??? TROUBLESHOOTING.md           ? Common issues
??? FAQ.md                       ? Questions & answers
??? DOCUMENTATION_SUMMARY.md     ? Documentation overview
```

---

## ?? Get Started Now!

**First time?** ? [Getting Started Guide](GETTING_STARTED.md)

**Ready to create?** ? [Script Examples](SCRIPT_EXAMPLES.md)

**Want to learn?** ? [rAthena Script Guide](RATHENA_SCRIPT_GUIDE.md)

**Need help?** ? [Index](INDEX.md) | [FAQ](FAQ.md) | [Troubleshooting](TROUBLESHOOTING.md)

---

**Happy scripting with BalrogNPC!** ???
